...
export default function ResourceLink({ resource }: ResourceLinkProps) {
    const getStyles = () => {
        switch (resource.type) {
            case 'pdf':
                return {
                    icon: 'picture_as_pdf',
                    containerClass: 'border-red-100 bg-gradient-to-r from-red-50 to-white hover:border-red-200 hover:shadow-red-100/50',
            ...
            case 'link':
                return {
                    icon: 'link',
                    containerClass: 'border-blue-100 bg-gradient-to-r from-blue-50 to-white hover:border-blue-200 hover:shadow-blue-100/50',
            ...
            default:
                return {
                    icon: 'description',
                    containerClass: 'border-gray-100 bg-gradient-to-r from-gray-50 to-white hover:border-gray-200 hover:shadow-gray-100/50',
    ...
    return (
        ...
        >
            <div className={`flex size-9 items-center justify-center rounded-lg bg-white shadow-sm ${styles.iconClass}`}>
                <Icon name={styles.icon} size={20} />
            </div>
            <div className="flex flex-col">
                <span className={`text-sm font-semibold transition-colors ${styles.textClass}`}>
                    {resource.title}
                </span>
