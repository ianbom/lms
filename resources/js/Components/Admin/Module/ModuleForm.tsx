...
export const createEmptyVideo = (id: number): VideoEntry => ({
    id,
    title: '',
    youtubeUrl: '',
    duration: '',
...
export default function ModuleForm({
    ...
    const updateVideos = (newVideos: VideoEntry[]) => {
        onDataChange({ ...data, videos: newVideos });
    };
                    ...
                    new window.YT.Player(containerId, {
                        ...
                        events: {
                            onReady: (event) => {
                                ...
                                onDataChange((prevData: ModuleFormData) => ({
                                    ...prevData,
    ...
    const handleFilesSelected = (videoId: number, fileList: FileList) => {
        ...
        onDataChange({ ...data, videos: updatedVideos });
    };
    ...
    return (
        <div
            className={`grid grid-cols-1 gap-8 ${isCompact ? 'lg:grid-cols-1' : 'lg:grid-cols-4'}`}
        >
            {/* Left Column - Forms */}
            <div
                className={`flex flex-col gap-6 ${isCompact ? '' : 'lg:col-span-3'}`}
            >
                ...
                <FormCard icon="folder" title="Informasi Modul">
                    <div className="flex flex-col gap-6">
                        ...
                        <RichTextEditor
                            ...
                            onChange={(val) =>
                                onDataChange({ ...data, description: val })
                            }
                ...
                <div className="space-y-4">
                    ...
                    {data.videos.map((video, index) => (
                        <div key={video.id}>
                            ...
                            {errors[`videos.${index}.youtube_url`] && (
                                <div className="mt-1 text-sm text-red-500">
                                    {errors[`videos.${index}.youtube_url`]}
                                </div>
                            )}
            ...
            {/* Right Column - Summary (only show if not compact) */}
            {!isCompact && (
