                    ...
                    <div key={module.id}>
                        ...
                        <div className="flex flex-col gap-2">
                            ...
                            {module.quizzes?.map((quiz) => {
                                ...
                                const attempted = quiz.attempts && quiz.attempts.length > 0;
                                    ...
                                    >
                                        <div className={`mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full ${
                                            passed
                                                ...
                                                : attempted
                                                    ? 'bg-red-100 border-2 border-red-400 text-red-400'
                ...
                <div className="border-t border-slate-100 bg-gradient-to-r from-amber-50 to-yellow-50 p-4">
                    <div className="mb-3 flex items-center gap-2">
                        <div className="flex h-8 w-8 items-center justify-center rounded-full bg-amber-100">
                            <Icon name="emoji_events" size={18} className="text-amber-600" />
                        </div>
                        <div>
                            <p className="text-sm font-bold text-slate-800">Selamat! 🎉</p>
                            <p className="text-xs text-slate-500">Anda telah menyelesaikan kelas ini</p>
                        </div>
                    </div>
                    <button
                ...
                <div className="border-t border-slate-100 bg-slate-50 p-4">
                    ...
                    <div className="space-y-2">
                        <div className="flex items-center gap-2 text-sm">
                            <Icon
                                name={certificateStatus.all_videos_completed ? 'check_circle' : 'radio_button_unchecked'}
                                size={16}
                                className={certificateStatus.all_videos_completed ? 'text-green-500' : 'text-slate-300'}
                            />
                            <span className={certificateStatus.all_videos_completed ? 'text-green-600' : 'text-slate-500'}>
                                Video: {certificateStatus.completed_videos}/{certificateStatus.total_videos}
                            </span>
                        </div>
                        {certificateStatus.total_quizzes > 0 && (
                            <div className="flex items-center gap-2 text-sm">
                                <Icon
                                    name={certificateStatus.all_quizzes_passed ? 'check_circle' : 'radio_button_unchecked'}
                                    size={16}
                                    className={certificateStatus.all_quizzes_passed ? 'text-green-500' : 'text-slate-300'}
                                />
                                <span className={certificateStatus.all_quizzes_passed ? 'text-green-600' : 'text-slate-500'}>
                                    Quiz lulus (min {certificateStatus.min_quiz_score}): {certificateStatus.passed_quizzes}/{certificateStatus.total_quizzes}
                                </span>
                            </div>
                        )}
                    </div>
